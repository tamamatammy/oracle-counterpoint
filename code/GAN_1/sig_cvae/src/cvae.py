...
class CVAE(object):
    ...
    def encoder(self, X_in, cond, input_dim):
        with tf.compat.v1.variable_scope("encoder", reuse=None):
            ...
            mn = tf.compat.v1.layers.dense(
                x, units=self.n_latent, activation=self.lrelu
            )
            sd = tf.compat.v1.layers.dense(
                x, units=self.n_latent, activation=self.lrelu
            )
            epsilon = tf.random.normal(tf.stack([tf.shape(input=x)[0], self.n_latent]))
            z = mn + tf.multiply(epsilon, tf.exp(sd / 2.0))
    ...
    def train(
        self, data, data_cond, n_epochs=10000, learning_rate=0.005, show_progress=False
    ):
        ...
        X_in = tf.compat.v1.placeholder(
            dtype=tf.float32, shape=[None, input_dim], name="X"
        )

        self.cond = tf.compat.v1.placeholder(
            dtype=tf.float32, shape=[None, dim_cond], name="c"
        )
        Y = tf.compat.v1.placeholder(
            dtype=tf.float32, shape=[None, input_dim], name="Y"
        )
        ...
        decoded_loss = tf.reduce_sum(
            input_tensor=tf.math.squared_difference(unreshaped, Y_flat), axis=1
        )
        latent_loss = -0.5 * tf.reduce_sum(
            input_tensor=1.0 + sd - tf.square(mn) - tf.exp(sd), axis=1
        )

        self.loss = tf.reduce_mean(
            input_tensor=(1 - self.alpha) * decoded_loss + self.alpha * latent_loss
        )
        ...
        for i in tqdm(range(n_epochs), desc="Training"):
            self.sess.run(
                optimizer, feed_dict={X_in: data, self.cond: data_cond, Y: data}
            )
            if not i % 1000 and show_progress:
                ls, d = self.sess.run(
                    [self.loss, self.dec],
                    feed_dict={X_in: data, self.cond: data_cond, Y: data},
                )
    ...
    def generate(self, cond, n_samples=None):
        ...
        samples = self.sess.run(
            self.dec, feed_dict={self.sampled: randoms, self.cond: cond}
        )
