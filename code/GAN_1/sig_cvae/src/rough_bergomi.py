...
def volterra_BM_path_chol(grid_points, M, H, T, rho):
    ...
    for j in range(grid_points - 1):
        for i in range(grid_points - 1):
            Sigma[i, j + ((grid_points - 1))] = (
                rho
                / (H + 0.5)
                * (
                    np.power(X[i], H + 0.5)
                    - np.power(X[i] - np.minimum(X[i], X[j]), H + 0.5)
                )
            )
            Sigma[i + (grid_points - 1), j] = (
                rho
                / (H + 0.5)
                * (
                    np.power(X[j], H + 0.5)
                    - np.power(X[j] - np.minimum(X[i], X[j]), H + 0.5)
                )
            )
    # Sigma(2,2)
    for j in range(grid_points - 1):
...
def rough_bergomi(grid_points, M, H, T, rho, xi0, nu, S0):
    ...
    C_H = np.power(
        2
        * H
        * special.gamma(3.0 / 2.0 - H)
        / special.gamma(H + 0.5)
        / special.gamma(2.0 - 2.0 * H),
        0.5,
    )
    Variance = xi0 * np.exp(
        2 * nu * C_H * V_path - np.power(nu * C_H, 2) * np.power(time_grid, 2 * H) / H
    )
    ...
    for i in range(grid_points - 1):
        log_stock[:, i + 1] = (
            log_stock[:, i]
            - 0.5 * Variance[:, i] * time_increment
            + np.power(Variance[:, i], 0.5) * brownian_increments[:, i]
        )
