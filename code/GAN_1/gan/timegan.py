...
import numpy as np
from model_core.gan.utils import (
    extract_time,
    rnn_cell,
    random_generator,
    batch_generator,
...
def recovery(H, T, module_name, hidden_dim, num_layers, dim):
...
def timegan(ori_data, parameters):
    ...
    X_tilde = recovery(H, T, module_name, hidden_dim, num_layers, dim)
    ...
    # Synthetic data
    X_hat = recovery(H_hat, T, module_name, hidden_dim, num_layers, dim)
    ...
    G_loss_V1 = tf.reduce_mean(
        input_tensor=tf.abs(
            tf.sqrt(tf.nn.moments(x=X_hat, axes=[0])[1] + 1e-6)
